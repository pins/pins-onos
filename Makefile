...
DOCKER_BUILD_ARGS        ?=
DOCKER_TAG               ?= ${VERSION}
ONOS_IMAGENAME           := ${DOCKER_REGISTRY}${DOCKER_REPOSITORY}pins-onos:${DOCKER_TAG}
...
build-local-apps:
	...
	cd sonic-onos-driver/pipeliner && mvn clean install
	cd sonic-onos-driver/driver && mvn clean install
